...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def _sample_visible_ids(self, visible, num_ids=1, min_id=None, max_id=None,
        ...
        valid_indices = [i for i in range(min_id, max_id) if visible[i]]
        if len(valid_indices) < num_ids:
            return None
        # ！！！核心修改！！！
        # 不要随机乱抓，要抓连续片段
        # 随机选择一个起始点，往后切 num_ids 个
        start_idx = random.randint(0, len(valid_indices) - num_ids)
        # 这里其实最好是直接取连续的帧号，但考虑到 visible 可能中间断开
        # 如果要求严格物理连续，需要判断 indices 也是连续的
        # 简化版：取排序后的 visible 帧（保持时序先后）
        # 确保取出来的是一段连续的时间序列
        return valid_ids[start_idx: start_idx + num_ids]
