...
def plot_draw_save(y, x, scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    plt.rcParams['text.usetex'] = False  # 改为 False 防止服务器没装 LaTeX 报错
    # plt.rcParams["font.family"] = "Times New Roman" # 注释掉防止缺少字体报错
    ...
    # ylabel = "%s"%(ylabel.replace('%','\%')) # 注释掉 LaTeX 格式
    xlim = plot_opts['xlim']
    ylim = plot_opts['ylim']
    ...
    print(f"\n{'-' * 20} {title} Results {'-' * 20}")
    # 按分数从高到低排序打印
    print_index = scores.argsort(descending=True)
    for idx in print_index:
        tracker = trackers[idx]
        name = get_tracker_display_name(tracker)
        score = scores[idx].item()
        print(f"Tracker: {name:<30} | Score: {score:.3f}")
    print(f"{'-' * 55}\n")
    # =========================================================
...
def plot_results(trackers, dataset, report_name, merge_results=False,
    ...
    print(f">>> [Debug] plot_results called with report_name: {report_name}")
    ...
    # Load pre-computed results
    result_plot_path = os.path.join(settings.result_plot_path, report_name)
    print(f">>> [Debug] Loading results from: {result_plot_path}")

    try:
        eval_data = check_and_load_precomputed_results(trackers, dataset, report_name, force_evaluation, **kwargs)
    except Exception as e:
        print(f">>> [Error] Failed to load/compute results: {e}")
        return

    print(">>> [Debug] Eval data loaded successfully.")
    ...
    print(f">>> [Debug] Found {num_valid} valid sequences out of {valid_sequence.shape[0]}")

    if num_valid == 0:
        print(">>> [Warning] No valid sequences found! Check dataset path or result files.")
        return
    ...
    if 'success' in plot_types:
        print(">>> [Debug] Processing Success Plot...")
        ave_success_rate_plot_overlap = torch.tensor(eval_data['ave_success_rate_plot_overlap'])
    ...
    if 'prec' in plot_types:
        print(">>> [Debug] Processing Precision Plot...")
        ave_success_rate_plot_center = torch.tensor(eval_data['ave_success_rate_plot_center'])
    ...
    if 'norm_prec' in plot_types:
        print(">>> [Debug] Processing Norm Precision Plot...")
        ave_success_rate_plot_center_norm = torch.tensor(eval_data['ave_success_rate_plot_center_norm'])
